...
//
//public class BlockProducerNode
//{
//
//
...
//
//            peersim.gossipsub.GossipSubProtocol.prefix = prefix;
//
...
//        public Object clone()
//        {
//            peersim.gossipsub.GossipSubProtocol cln = new peersim.gossipsub.GossipSubProtocol(peersim.gossipsub.GossipSubProtocol.prefix);
//            return cln;
//        }
//
...
//            int bandwidth = 0;
//            if (this.nodeId == ((peersim.gossipsub.GossipSubProtocol) (CustomDistribution.blockProposerNode.getProtocol(gossipSubId))).nodeId)
//            {
...
//                Message request = this.createMessage(m.id, Message.MSG_IWANT, m.src, m.messageTopicID, "",m.isRow,m.rowOrColumnNumber,m.partNumber,m.timestamp);
...
//                Message response = this.createMessage(m.id, Message.MSG_DATA, m.src, m.messageTopicID, responseData, m.isRow, m.rowOrColumnNumber, m.partNumber, m.timestamp);
//
//                // send data/response to IWANT Request
//                publishMessage(response, m.src, myPid);
//            }
...
//            int n = 8; //Need to make this configurable-------------------
...
//            List<Integer> uniquePartNumbers = allParts.subList(0, 3);
//
//            Node n1, n2, n3;
...
//                dest1 = ((peersim.gossipsub.GossipSubProtocol) (n1.getProtocol(gossipSubId))).nodeId;
//                requestPart1 = this.createMessage(-1, Message.MSG_IWANT, dest1, m.messageTopicID, "", m.isRow, m.rowOrColumnNumber, uniquePartNumbers.get(0), 0);
//
//                n2 = topics.get(m.messageTopicID).topicMembers.get(uniquePartNumbers.get(1));
//                dest2 = ((peersim.gossipsub.GossipSubProtocol) (n2.getProtocol(gossipSubId))).nodeId;
//                requestPart2 = this.createMessage(-1, Message.MSG_IWANT, dest2, m.messageTopicID, "", m.isRow, m.rowOrColumnNumber, uniquePartNumbers.get(1), 0);
//
//                n3 = topics.get(m.messageTopicID).topicMembers.get(uniquePartNumbers.get(0));
//                dest3 = ((peersim.gossipsub.GossipSubProtocol) (n3.getProtocol(gossipSubId))).nodeId;
//                requestPart3 = this.createMessage(-1, Message.MSG_IWANT, dest3, m.messageTopicID, "", m.isRow, m.rowOrColumnNumber, uniquePartNumbers.get(2), 0);
//
//            } else {
...
//            if (m.src == ((peersim.gossipsub.GossipSubProtocol) (CustomDistribution.blockProposerNode.getProtocol(gossipSubId))).nodeId)
//            {
...
//                        Message responseWithMetaData = this.createMessage(m.id, Message.MSG_IHAVE, m.dest, m.messageTopicID, "",m.isRow,m.rowOrColumnNumber,m.partNumber,m.timestamp); // Temporalily setting the dummy .dest. It will set correctly in the sendMessageToPeers Function
...
//            {
////            System.out.println("Node ID: "+this.nodeId+" recieved the data from BP at "+CommonState.getTime());
//                BigInteger blockProposerId = ((peersim.gossipsub.GossipSubProtocol) (CustomDistribution.blockProposerNode.getProtocol(gossipSubId))).nodeId;
//                if (messageCache.size() > MESSAGE_CACHE_SIZE)// Checking if the size of cache has exceeded the max value
//                {
...
//                partRequestCounter--;
//            }
//            if(partRequestCounter==0)
//            {
//                startSampling();
//            }
//
//            //Sending the message with data to all the peers/local mesh
//            Message responseWithData = this.createMessage(m.id, Message.MSG_DATA, m.dest, m.messageTopicID, m.body,m.isRow,m.rowOrColumnNumber,m.partNumber,m.timestamp); // Temporalily setting the dummy .dest. It will set correctly in the sendMessageToPeers Function
...
//        private Message createMessage(long id, int type, BigInteger dest, String topicID, Object body, boolean isRow, int RowOrColNum, int partNum, long timeStamp) {
//            Message msg;
//            if (id == -1) //Create a new message with new message Id
//            {
//                msg = new Message(type, isRow, RowOrColNum, partNum);
//                msg.timestamp = CommonState.getTime(); // Getting the current time/ time at which the message was created
//            } else {
//                msg = new Message(id, type, isRow, RowOrColNum, partNum);
...
//                destId = ((peersim.gossipsub.GossipSubProtocol) (topicToSubscribe.topicMembers.get(nodeIndex).getProtocol(gossipSubId))).nodeId;
//            } else { // Column sampling
//                // Nodes 8-15 handle columns
//                destId = ((peersim.gossipsub.GossipSubProtocol) (topicToSubscribe.topicMembers.get(nodeIndex + 8).getProtocol(gossipSubId))).nodeId;
//            }
...
//            }
//
//            Message sampleReqMes = createMessage(-1, Message.MSG_SAMPLE_DATA_REQUEST, destId, topicToSubscribe.topicID, Integer.toString(SamplingIdx),rowOrColDecider==0,rowOrColNo,-1,0);//Need to change/calculate the part the partNo. Just added some random value atm
//
...
//                        Message sampleResponse = createMessage(m.id,Message.MSG_SAMPLE_DATA_RESPONSE,m.src,m.messageTopicID,Integer.toString(sampleDataResp) ,m.isRow,m.rowOrColumnNumber,m.partNumber,m.timestamp);
...
//                        {
//                            byte[][] data = (byte[][]) curMessage.body;//Currently we are just sending random sample response as we are not forming the whole row/col over here but in reality the will form the whole row/col if it has 50% cells
//                            byte sampleDataResp = data[0][0];
//                            Message sampleResponse = createMessage(m.id,Message.MSG_SAMPLE_DATA_RESPONSE,m.src,m.messageTopicID,Integer.toString(sampleDataResp) ,m.isRow,m.rowOrColumnNumber,m.partNumber,m.timestamp);
//                            this.publishMessage(sampleResponse,m.src,gossipSubId);
//                            break;
//                        }
...
//                        Message msgToResend = this.createMessage(-1,sentMsg.type,sentMsg.dest,sentMsg.messageTopicID,sentMsg.body,sentMsg.isRow,sentMsg.rowOrColumnNumber,sentMsg.partNumber,0);
...
//                        Message newMessage = new Message(3, rowToSend,true,rowNumber,-1);
...
//
//                        BigInteger destID = ((peersim.gossipsub.GossipSubProtocol) n.getProtocol(gossipSubId)).getNodeId();
//                        Message newMessage = new Message(3, colToSend,false,columnNumber,-1);
//                        newMessage.src = this.getNodeId();
//                        newMessage.messageTopicID = topicEntry.getValue().topicID;
//                        // System.out.println("NodeId for given topic is:"+destID);
//                        newMessage.dest = destID;
...
//            int numberOfDivisions = 8;// Equals to the number of rows/columns per topic
//            int partSize = 512 / 8;
//
//            Block b = new Block(Configuration.getInt("NUMBER_OF_ROWS"), Configuration.getInt("NUMBER_OF_COLUMNS")); // Creating a block
//            peersim.gossipsub.GossipSubProtocol iGossipBlockProposer = (peersim.gossipsub.GossipSubProtocol) (CustomDistribution.blockProposerNode.getProtocol(gossipSubId)); // Get the protocol instance of the block proposer
...
////                    System.out.println("*******");
//                        rowsToSend[0] = Arrays.copyOfRange(b.getRowData(curTopicNumber * 8), start, end);
//                        rowsToSend[1] = Arrays.copyOfRange(b.getRowData(curTopicNumber * 8 + 1), start, end);
//                        rowsToSend[2] = Arrays.copyOfRange(b.getRowData(curTopicNumber * 8 + 2), start, end);
//                        rowsToSend[3] = Arrays.copyOfRange(b.getRowData(curTopicNumber * 8 + 3), start, end);
//                        rowsToSend[4] = Arrays.copyOfRange(b.getRowData(curTopicNumber * 8 + 4), start, end);
//                        rowsToSend[5] = Arrays.copyOfRange(b.getRowData(curTopicNumber * 8 + 5), start, end);
//                        rowsToSend[6] = Arrays.copyOfRange(b.getRowData(curTopicNumber * 8 + 6), start, end);
//                        rowsToSend[7] = Arrays.copyOfRange(b.getRowData(curTopicNumber * 8 + 7), start, end);
////                    System.out.println(Arrays.toString(b.getRowData(curTopicNumber * 8 + 1)));
//                        BigInteger destID = ((peersim.gossipsub.GossipSubProtocol) n.getProtocol(gossipSubId)).getNodeId();
//                        Message newMessage = new Message(3, rowsToSend, true, -1, rowPart);
...
//                        int end = start + partSize;
//                        colsToSend[0] = Arrays.copyOfRange(b.getColumnData(curTopicNumber * 8), start, end);
//                        colsToSend[1] = Arrays.copyOfRange(b.getColumnData(curTopicNumber * 8 + 1), start, end);
//                        colsToSend[2] = Arrays.copyOfRange(b.getColumnData(curTopicNumber * 8 + 2), start, end);
//                        colsToSend[3] = Arrays.copyOfRange(b.getColumnData(curTopicNumber * 8 + 3), start, end);
//                        colsToSend[4] = Arrays.copyOfRange(b.getColumnData(curTopicNumber * 8 + 4), start, end);
//                        colsToSend[5] = Arrays.copyOfRange(b.getColumnData(curTopicNumber * 8 + 5), start, end);
//                        colsToSend[6] = Arrays.copyOfRange(b.getColumnData(curTopicNumber * 8 + 6), start, end);
//                        colsToSend[7] = Arrays.copyOfRange(b.getColumnData(curTopicNumber * 8 + 7), start, end);
//
//                        BigInteger destID = ((peersim.gossipsub.GossipSubProtocol) n.getProtocol(gossipSubId)).getNodeId();
//                        Message newMessage = new Message(3, colsToSend, false, -1, colPart);
//                        newMessage.src = this.getNodeId();
//                        newMessage.messageTopicID = topicEntry.getValue().topicID;
//                        newMessage.dest = destID;
//
...
////The BP sends the half of the whole row/col which is divided into 8 parts
//
...
//                    BigInteger destID = ((peersim.gossipsub.GossipSubProtocol) n.getProtocol(gossipSubId)).getNodeId();
////                    System.out.println("Row number is "+rowNumber);
//                    Message newMessage = new Message(3, rowsToSend,true,rowNumber,-1);
//                    newMessage.src = this.getNodeId();
//                    newMessage.messageTopicID = topicEntry.getValue().topicID;
//                    // System.out.println("NodeId for given topic is:"+destID);
//                    newMessage.dest = destID;
...
//
//                    BigInteger destID = ((peersim.gossipsub.GossipSubProtocol) n.getProtocol(gossipSubId)).getNodeId();
//                    Message newMessage = new Message(3,colsToSend ,false,columnNumber,-1);
//                    newMessage.src = this.getNodeId();
//                    newMessage.messageTopicID = topicEntry.getValue().topicID;
//                    // System.out.println("NodeId for given topic is:"+destID);
//                    newMessage.dest = destID;
...
//    }
//    }
//    //Each topic will have 16 nodes
//    //Each node will either hold 2 rows or 2 cols.
//    //Like the first 2 nodes will hold the same 2 rows. Next 2 nodes will hold the same 2 rows.
//    //Over here the Block Proposer is not sending the whole row/col as the the node can reconstruct the whole row/col if it have atleast 50% of the cells
//
//
