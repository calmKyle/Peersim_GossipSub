...
public class GossipSubObserver implements Control {
        ...
        public boolean execute() {
                ...
                try (BufferedWriter writer = new BufferedWriter(new FileWriter(filePath, append))) {
                        ...
                        if (!append) {
                                writer.write("Time,Node ID,Seed Arrival Times,Seed RTT Times,Min Seed RTT,Avg Seed RTT,Max Seed RTT,"
                        ...
                        for (Node nd : CustomDistribution.networkNodes.values()) {
                                ...
                                String metrics4 = String.format(
                                                "[average bandwidth=%f]",
                                                ((double) protocol.totalDataTransmitted
                                                                / (double) (protocol.totalTransmissionTime) * 1000));
                                ...
                                String seedPartArrivalTimes = proposerStratergy == 2
                                                && !protocol.seedPartArrivalTimeFromPeer.isEmpty()
                                                                ? Arrays.toString(protocol.seedPartArrivalTimeFromPeer
                                                                                .toArray()).replace("[", "")
                                                                                .replace("]", "").replace(",", ";")
                                                                : "[]";

                                String seedPartDelayTimes = proposerStratergy == 2
                                                && !protocol.seedPartDelayTimeFromPeer.isEmpty()
                                                                ? Arrays.toString(protocol.seedPartDelayTimeFromPeer
                                                                                .toArray()).replace("[", "")
                                                                                .replace("]", "").replace(",", ";")
                                                                : "[]";

                                String metrics = String.format(
                                                "%d,%d,%s,%s,%f,%f,%f,%d,%d,%d,%s,%s,%f,%f,%f,%d,%s,%s,%f,%f,%f",
                                                CommonState.getTime(),
                                                count,
                                                seedArrivalTimes,
                                                ...
                                                avgSampleRTT,
                                                maxSampleRTT,
                                                proposerStratergy == 2 ? protocol.NoOfSeedPartsRecieved : 0,
                                                seedPartArrivalTimes,
                                                seedPartDelayTimes,
                                                proposerStratergy == 2 ? protocol.seedPartArrivalTimeStore.getMin()
                                                                : 0.0,
                                                proposerStratergy == 2 ? protocol.seedPartArrivalTimeStore.getAverage()
                                                                : 0.0,
                                                proposerStratergy == 2 ? protocol.seedPartArrivalTimeStore.getMax()
                                                                : 0.0,
                                                (double) protocol.totalDataTransmitted
                                                                / (double) (protocol.totalTransmissionTime * 1000));
...
// 1. Number of copies of each row/col that are distributed by the block
// producer (LEFT)
// 6. If whole/complete row/col to be sent to individual node or not (LEFT)

// 1. RTT(Round trip time)
// 2. Latency(Time it took to recieve the data/sample after the request message
// was sent)
// 3. Time it takes distribute the rows/cols and the time it takes for sampling
// 4. Number of messages delivered(show stats for: type of message, number of
// hops)

// 5. Number of Retransmissions (no need as the transport protocol is
// reliable/no data loss )
// 6. Number of packet/sample loss (no need as the transport protocol is
// reliable/no data loss )
